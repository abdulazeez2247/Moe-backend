...
const parseXML = async (filePath) => {
    try {
        const data = await fs.readFile(filePath, 'utf8');
        const simplifiedXml = data.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
...
const parseMozaikFile = async (filePath, originalName) => {
    ...
    try {
        ...
        switch (ext) {
            ...
            case '.cabx':
            case '.mzb':
                extractedText = 'Binary project file uploaded. Filename and type provided as context.';
                break;
        }

        const truncatedText = extractedText.length > 12000 ? extractedText.substring(0, 12000) + '...' : extractedText;
