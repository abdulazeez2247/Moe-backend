...
const buildSystemPrompt = (platform, version) => {
    ...
    return `${basePrompt} The user is specifically using ${platform} ${version ? 'version ' + version : ''}. Tailor your instructions to this software's interface and terminology.`;
};
...
const generateAnswer = async (questionText, platform, version, model, fileContext = null) => {
    try {
        const systemPrompt = buildSystemPrompt(platform, version);
        const userMessage = fileContext ? `${questionText}\n\nRelevant file context:\n${fileContext}` : questionText;
