...
questionSchema.statics.findByQuestion = function (questionText, platform, version) {
  const baseSlug = slugify(questionText, { lower: true, strict: true, remove: /[*+~.()'"!:@]/g });
...
questionSchema.statics.createFromQuestion = function (questionText, platform, version, answer, modelUsed, tokens, sources) {
  const baseSlug = slugify(questionText, { lower: true, strict: true, remove: /[*+~.()'"!:@]/g });
  const platformSlug = slugify(platform, { lower: true, strict: true });
  const versionSlug = version ? slugify(version, { lower: true, strict: true }) : 'generic';
  const canonicalId = `${platformSlug}:${versionSlug}:${baseSlug}`;
