...
const createPaymentIntent = async (req, res, next) => {
    try {
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            ...
            description: `Payment for ${metadata.productName || 'product'} by ${user.email}`,
            receipt_email: process.env.NODE_ENV === 'production' ? user.email : undefined,
            statement_descriptor: process.env.NODE_ENV === 'production' ? 'YOURCOMPANY' : 'TESTCOMPANY',
            statement_descriptor_suffix: metadata.productName ? metadata.productName.substring(0, 12) : 'PRODUCT'
        });

        console.log(`PaymentIntent created: ${paymentIntent.id} for user: ${user.email}, amount: ${amount/100} ${currency}`);
...
const handleInvoicePaymentSucceeded = async (successfulInvoice) => {
    const successfulUser = await User.findOne({ customerId: successfulInvoice.customer });
    if (successfulUser) {
        await Payment.create({
