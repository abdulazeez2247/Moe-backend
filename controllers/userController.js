...
const updateUserProfile = async (req, res, next) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(req.user.id, { seesAds }, { new: true, runValidators: true });
        res.status(200).json({ 
            status: 'success', 
            data: { 
                user: updatedUser,
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                monthlyLimit: user.plan === 'free' ? (process.env.FREE_DAILY_LIMIT || 5) * 30 : getPlanLimit(user.plan),
                plan: user.plan,
                assistantMessageCount: user.assistantMessageCount
            }
        });
